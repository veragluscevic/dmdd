...
def dRdQ(np.ndarray[DTYPE_t] Er, DTYPE_t mass=50., np.ndarray[DTYPE_t] c1p=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c3p=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c4p=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c5p=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c6p=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c7p=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c8p=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c9p=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c10p=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c11p=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c12p=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c13p=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c14p=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c15p=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c1n=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c3n=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c4n=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c5n=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c6n=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c7n=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c8n=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c9n=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c10n=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c11n=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c12n=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c13n=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c14n=np.array([0.,0.,0.]), np.ndarray[DTYPE_t] c15n=np.array([0.,0.,0.]), DTYPE_t v_lag=220., DTYPE_t v_rms=220., DTYPE_t v_esc=544., str element='xenon', DTYPE_t rho_x=0.3, DTYPE_t c_scale=c_scale_global):
    ...
    for i in range(npts):
        ...
        ff_M_v2q2 = formgenNR.factor_M(element_name,y_harm, c5p[0]*c5p[0]/4.+2.*c8p[1]*c8p[0]/4.*mN**2./mass**2, c5p[0]*c5n[0]/4.+(c8p[1]*c8n[0]+c8p[0]*c8n[1])/4.*mN**2./mass**2, c5n[0]*c5p[0]/4.+(c8n[1]*c8p[0]+c8n[0]*c8p[1])/4.*mN**2./mass**2, c5n[0]*c5n[0]/4.+2.*c8n[1]*c8n[0]/4.*mN**2./mass**2)
        ...
        ff_SigPP_v2q2 = formgenNR.factor_SigPP(element_name,y_harm, c13p[0]*c13p[0]/16.+2.*c12p[1]*c12p[0]/16.*mN**2/mass**2, c13p[0]*c13n[0]/16.+(c12p[1]*c12n[0]+c12p[0]*c12n[1])/16.*mN**2/mass**2, c13n[0]*c13p[0]/16.+(c12n[1]*c12p[0]+c12n[0]*c12p[1])/16.*mN**2/mass**2, c13n[0]*c13n[0]/16.+2.*c12n[1]*c12n[0]/16.*mN**2/mass**2)
        ff_SigPP_q4 = formgenNR.factor_SigPP(element_name,y_harm, (c4p[1]*c4p[1]+2*c4p[2]*c4p[0])*mN**4./mass**4./16.+(2.*c10p[1]*c10p[0]/4.+2.*(c4p[1]*c6p[0]+c6p[1]*c4p[0])/16.-2.*c12p[1]*c12p[0]*mN**2./(64.*m_reduced_sq))*mN**2./mass**2.+c6p[0]*c6p[0]/16.-c13p[0]*c13p[0]*mN**2./(64.*m_reduced_sq), (c4p[1]*c4n[1]+c4p[2]*c4n[0]+c4p[0]*c4n[2])*mN**4./mass**4./16.+((c10p[1]*c10n[0]+c10p[0]*c10n[1])/4.+(c4p[1]*c6n[0]+c4p[0]*c6n[1]+c6p[1]*c4n[0]+c6p[0]*c4n[1])/16.-(c12p[1]*c12n[0]+c12p[0]*c12n[1])*mN**2./(64.*m_reduced_sq))*mN**2./mass**2.+c6p[0]*c6n[0]/16.-c13p[0]*c13n[0]*mN**2./(64.*m_reduced_sq), (c4n[1]*c4p[1]+c4n[2]*c4p[0]+c4n[0]*c4p[2])*mN**4./mass**4./16.+((c10n[1]*c10p[0]+c10n[0]*c10p[1])/4.+(c4n[1]*c6p[0]+c4n[0]*c6p[1]+c6n[1]*c4p[0]+c6n[0]*c4p[1])/16.-(c12n[1]*c12p[0]+c12n[0]*c12p[1])*mN**2./(64.*m_reduced_sq))*mN**2./mass**2.+c6n[0]*c6p[0]/16.-c13n[0]*c13p[0]*mN**2./(64.*m_reduced_sq), (c4n[1]*c4n[1]+2*c4n[2]*c4n[0])*mN**4./mass**4./16.+(2.*c10n[1]*c10n[0]/4.+2.*(c4n[1]*c6n[0]+c6n[1]*c4n[0])/16.-2.*c12n[1]*c12n[0]*mN**2./(64.*m_reduced_sq))*mN**2./mass**2.+c6n[0]*c6n[0]/16.-c13n[0]*c13n[0]*mN**2./(64.*m_reduced_sq))
        ...
        ff_SigP_v2q2 = formgenNR.factor_SigP(element_name,y_harm, c3p[0]*c3p[0]/8.+(c14p[0]*c14p[0]-c12p[0]*c15p[0]-c15p[0]*c12p[0])/32.+2.*(c12p[1]*c12p[0]/32.+c7p[1]*c7p[0]/8.)*mN**2/mass**2, c3p[0]*c3n[0]/8.+(c14p[0]*c14n[0]-c12p[0]*c15n[0]-c15p[0]*c12n[0])/32.+((c12p[1]*c12n[0]+c12p[0]*c12n[1])/32.+(c7p[1]*c7n[0]+c7p[0]*c7n[1])/8.)*mN**2/mass**2, c3n[0]*c3p[0]/8.+(c14n[0]*c14p[0]-c12n[0]*c15p[0]-c15n[0]*c12p[0])/32.+((c12n[1]*c12p[0]+c12n[0]*c12p[1])/32.+(c7n[1]*c7p[0]+c7n[0]*c7p[1])/8.)*mN**2/mass**2, c3n[0]*c3n[0]/8.+(c14n[0]*c14n[0]-c12n[0]*c15n[0]-c15n[0]*c12n[0])/32.+2.*(c12n[1]*c12n[0]/32.+c7n[1]*c7n[0]/8.)*mN**2/mass**2)
        ...
        ff_SigPDelta_q4 = formgenNR.factor_SigPDelta(element_name,y_harm, (c5p[1]*c4p[0]+c5p[0]*c4p[1]-c8p[1]*c9p[0]-c8p[0]*c9p[1])/4.*mN**2/mass**2, (c5p[1]*c4n[0]+c5p[0]*c4n[1]-c8p[1]*c9n[0]-c8p[0]*c9n[1])/4.*mN**2/mass**2, (c5n[1]*c4p[0]+c5n[0]*c4p[1]-c8n[1]*c9p[0]-c8n[0]*c9p[1])/4.*mN**2/mass**2, (c5n[1]*c4n[0]+c5n[0]*c4n[1]-c8n[1]*c9n[0]-c8n[0]*c9n[1])/4.*mN**2/mass**2)
